...
public class CodingQuestionService {
    ...
    public List<Question> getCodingQuestionsByLanguage(String languageName, int page, int size) {
        String tableName = languageName.toLowerCase();
        System.out.println("pageaja: " + page + size);

        String queryString = "SELECT * FROM " + tableName+"code" + " ORDER BY id OFFSET :offset ROWS FETCH NEXT :size ROWS ONLY";
